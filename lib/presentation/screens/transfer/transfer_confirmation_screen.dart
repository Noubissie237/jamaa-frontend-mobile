...
class _TransferConfirmationScreenState extends State<TransferConfirmationScreen> {
  ...
  Future<void> _processTransfer() async {
    if (_pinController.text.length != 4) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Veuillez saisir votre code PIN'),
        ),
      );
      return;
    }
    ...
    try {
      ...
      await context.read<TransactionProvider>().createTransaction(
        ...
        recipientPhone: _getRecipientPhone(),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: () {
                ...
                context.go('/main'); // Retourner au dashboard
              },
  ...
  String _getTransactionTitle() {
    ...
    switch (transferType) {
      case 'user':
        return 'Transfert vers ${widget.transferData['recipient']}';
      case 'bank':
        return 'Transfert vers ${widget.transferData['bank']}';
      case 'mobile':
        return 'Transfert ${widget.transferData['operator']}';
      default:
        return 'Transfert';
    }
  }
  ...
  String _getTransactionDescription() {
    final reason = widget.transferData['reason'] as String?;
    return reason?.isNotEmpty == true ? reason! : 'Transfert d\'argent';
  }
  ...
  String? _getRecipientName() {
    ...
    if (transferType == 'user') {
      return widget.transferData['recipient'];
    }
    return null;
  }
  ...
  String? _getRecipientPhone() {
    ...
    if (transferType == 'mobile') {
      return widget.transferData['phoneNumber'];
    }
    return null;
  }
  ...
  String? _getBankName() {
    ...
    if (transferType == 'bank') {
      return widget.transferData['bank'];
    }
    return null;
  }
}